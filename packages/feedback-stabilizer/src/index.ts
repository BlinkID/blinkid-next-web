...
import { getKeyWithHighestValue, StringKeyOf } from "./utils";
...
export class FeedbackStabilizer<SdkSpecificStateMap extends UiStateMap> {
  ...
  getNewUiState(
    ...
    // single emmited events are placed in a different queue and emmited once the previous event is done
    if (uiStateEvent.singleEmit) {
      this.singleEventQueue.push(uiStateEvent);
    }

    // if there is a single event in the queue, show it
    if (this.canShowNewUiState() && this.singleEventQueue.length > 0) {
    ...
    for (const trackKey in this.scoreBoard) {
      ...
      // TODO: See if relaative amount makes sense
