...
import { getFPSFromConstraints } from "./utils";
...
class MediaMocker {
  ...
  createFakeStream(constraints: MediaStreamConstraints): Promise<MediaStream> {
    ...
    defineProperty(fakeTrack, "getCapabilities", () => {
      return mockCamera.streamCapabilities;
    });

    // just a stub for now
    defineProperty(fakeTrack, "getSettings", () => {
      return {
        deviceId: mockCamera.inputDeviceInfo.deviceId,
  ...
  createFakeTrack(): MediaStreamTrack {
    ...
    const fakeTrack: MediaStreamTrack = {
      ...
      getSettings: () => {
        return {
          deviceId: mockCamera.inputDeviceInfo.deviceId,
          frameRate: getFPSFromConstraints(constraints),
          width: 1280,
          height: 720,
        };
      },
